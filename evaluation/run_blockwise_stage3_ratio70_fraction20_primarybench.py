...
POSTPROCESS_SCRIPT = REPO_ROOT / "evaluation" / "postprocess_blockwise_stage3_ratio70_fraction20_primarybench.py"
PYTHON_BIN = REPO_ROOT / ".venv" / "bin" / "python"
MODEL = os.environ.get("MODEL", "/Tan/model/Llama-3.1-8B-Instruct")
DEVICE = os.environ.get("DEVICE", "cuda:0")
GPU_INDEX = int(os.environ.get("GPU_INDEX", "0"))
MIN_FREE_MB = int(os.environ.get("MIN_FREE_MB", "30000"))
POLL_SECONDS = int(os.environ.get("POLL_SECONDS", "60"))
MAX_RETRIES = int(os.environ.get("MAX_RETRIES", "3"))
PG19_SOURCE_DATASET = os.environ.get("PG19_SOURCE_DATASET", "pg19")
...
def _compact_dir_name(components: list[str], max_len: int = 180) -> str:
    ...
    compact_components = [_compact_component(str(component), max_len=20) for component in components if component]
    fallback = "__".join(compact_components[:8]) + f"__{digest}"
    return fallback if len(fallback) <= max_len else digest
...
def _normalize_task_filter(task_filter: Any) -> list[str]:
    ...
    if isinstance(task_filter, (list, tuple)):
        ...
        for item in task_filter:
            out.extend(_normalize_task_filter(item))
        return out
...
def build_results_base_dir(args_map: dict[str, Any]) -> Path:
    components = [
        args_map["dataset"],
        str(args_map.get("data_dir") or ""),
        MODEL.replace("/", "--"),
        args_map["press_name"],
        f"{float(args_map['compression_ratio']):.2f}",
    ]
    if float(args_map.get("fraction", 1.0)) < 1.0:
        components.append(f"fraction{float(args_map['fraction']):.3f}")
    if args_map.get("max_context_length") is not None:
        components.append(f"max_context{args_map['max_context_length']}")
    if args_map.get("query_aware") in {"true", "True", True}:
    ...
    if args_map.get("needle_depth") is not None and args_map["dataset"] == "needle_in_haystack":
        components.append(f"needle_depth{_canonicalize_literal(args_map['needle_depth'])}")
    if args_map.get("pg19_target_tokens") is not None and args_map["dataset"] == "pg19":
        components.append(f"pg19target{args_map['pg19_target_tokens']}")
    if args_map.get("pg19_source_dataset") is not None and args_map["dataset"] == "pg19":
        components.append(f"pg19src{str(args_map['pg19_source_dataset']).replace('/', '--')}")
...
def run_job(job: Job) -> bool:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if reason in {"oom", "killed"}:
            time.sleep(POLL_SECONDS)
        elif reason in {"network", "pg19_network", "cache_mismatch"}:
            time.sleep(20)
        else:
            time.sleep(5)
...
def pg19_args() -> list[str]:
    return [
        "--dataset", "pg19",
        "--pg19_source_dataset", PG19_SOURCE_DATASET,
...
def build_jobs() -> list[Job]:
    ...
    dataset_specs = [
        ...
        ("needle_in_haystack:16384", needle_args()),
        ("pg19:test", pg19_args()),
    ]

    methods = [
        ("blockwise_main", blockwise_common_args("mean_plus_norm_topk_mean", "key_norm", "max", "uniform_mean")),
        ("blockwise_multi_rep", blockwise_common_args("multi_rep_max", "key_norm", "max", "uniform_mean")),
        ("blockwise_adaptive_fusion_v1", blockwise_common_args("adaptive_fusion_v1", "key_norm", "max", "uniform_mean")),
        ("blockwise_multi_rep_diverse_v1", blockwise_common_args("multi_rep_max", "key_norm_diverse", "max", "uniform_mean")),
        ("chunkkv_prefill", ["--press_name", "chunkkv_prefill_per_layer", "--block_size", "16"]),
    ]
...
def run_postprocess() -> None:
    log("Running stage3 postprocess")
...
def main() -> int:
    ...
    try:
        ...
        run_postprocess()
    log(f"Stage3 controller finished for {EXPERIMENT_NAME}")
    return 0
