...
def route_args(route: str, budget: int) -> tuple[list[str], dict[str, Any]]:
    ...
    if route == "dense_prefill__compute_cold":
        return [
            ...
            "decode_cold_block_budget": budget,
        }
    if route == "blockwise_prefill__permanent":
        return [
            "--compression_ratio",
            "0.3",
            "--dual_phase_mode",
    ...
    if route == "blockwise_prefill__compute_cold":
        return [
            "--compression_ratio",
            "0.3",
            "--dual_phase_mode",
...
def make_jobs() -> list[Job]:
    ...
    longbench_routes = [
        "dense_prefill__permanent",
        "dense_prefill__compute_cold",
        "blockwise_prefill__permanent",
        "blockwise_prefill__compute_cold",
    ]
    for task in LONG_BENCH_TASKS:
        for budget in BUDGETS:
            for route in longbench_routes:
                extra_args, match = route_args(route, budget)
                args = common_args() + [
                    "--dataset",
                    "longbench",
                    "--data_dir",
                ...
                jobs.append(
                    Job(
                        job_id=f"longbench:{task}__{route}__budget{budget}",
                        cli_args=args,
                        match_fields={
                            "dataset": "longbench",
                            "data_dir": task,
    ...
    for budget in RULER_BUDGETS:
        for route in ruler_routes:
            extra_args, match = route_args(route, budget)
            args = common_args() + [
                "--dataset",
                "ruler",
            ...
            jobs.append(
                Job(
                    job_id=f"ruler:4096__{route}__budget{budget}",
                    cli_args=args,
                    match_fields={
                        "dataset": "ruler",
                        "data_dir": "4096",
