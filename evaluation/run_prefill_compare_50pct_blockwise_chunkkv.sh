...
OUTPUT_DIR="${OUTPUT_DIR:-./results_prefill_compare_50pct_blockwise_chunkkv}"
...
run_eval() {
  ...
  echo "Running dataset=${dataset} data_dir=${data_dir} press=${press} ratio=${ratio} device=${GPU}"
  if [[ -n "${data_dir}" ]]; then
...
run_dataset_grid() {
  ...
  for press in "${PRESSES[@]}"; do
    for ratio in "${RATIOS[@]}"; do
      run_eval "${dataset}" "${data_dir}" "${press}" "${ratio}" "${extra_args[@]}"
    done
  done
}
