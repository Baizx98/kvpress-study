...
EXP="ATC26_blockwise_temporal_index_similarity"
RUN_TAG="${RUN_TAG:-full}"
DECODE_STEPS="${DECODE_STEPS:-512}"
...
LOG_DIR="$ART/logs"
WATCHDOG_LOG="$LOG_DIR/ATC26_temporal_similarity_watchdog.log"
RUN_LOG="$LOG_DIR/ATC26_temporal_similarity_run.log"
COMMAND_FILE="$ART/ATC26_temporal_similarity_command.txt"
HEARTBEAT="$ART/ATC26_temporal_similarity_heartbeat.json"
AGG="$ART/ATC26_temporal_similarity_aggregate.json"
PID_FILE="$ART/ATC26_temporal_similarity_watchdog.pid"

mkdir -p "$LOG_DIR" "$ART/raw" "$ART/scores"
echo "$$" > "$PID_FILE"
